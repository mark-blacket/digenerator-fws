#include "digenerator.h"

static uint16_t _rnd = 0xAB;

const uint8_t reverseArray[256] PROGMEM = {
     
}; 

uint16_t random()
{
    uint16_t temp = ~(_rnd >> 15) ^ (_rnd >> 14) ^ ((_rnd >> 11) & (_rnd >> 7)) + 7;
    _rnd = _rnd << 1;
    return _rnd |= temp & 0x01;
}

uint8_t reverse(uint8_t val)
{
    return pgm_read_byte(&(reverseArray[val]));
}

uint8_t div3(uint8_t n)
{
    uint8_t sum = 0, tmp = 0;
    while (sum > 3) {
        sum += (tmp = (n >> 2));
        n = tmp + (n & 3);
    }
    return (n == 3) ? (sum + 1) : sum;
}

void dac(unsigned char val)
{
    val = reverse(val);
    PORTD &= 0x0F;
    PORTB &= 0xF0;
    PORTD |= (val) & 0xF0;
    PORTB |= (val) & 0x0F;
}

void setup()
{
    DDRD = 0xF0;
    DDRB = 0x1F;
    DDRC = 0x00;
    
    EIMSK |= B(INT0) | B(INT1);  // enable both interrupts
    EICRA |= B(ISC11);           // rising edge
    sei();

    ADMUX  |= B(REFS0) | B(ADLAR) | B(MUX2) | B(MUX1); // avcc, left-align, channel 6
    ADCSRA |= B(ADEN)  | B(ADATE) | B(ADPS2);          // freerun, /16 prescale, start
    ADCSRA |= B(ADSC);
}
